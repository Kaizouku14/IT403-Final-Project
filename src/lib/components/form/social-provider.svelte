<script lang="ts">
	...
	import { socialProviders, pageRoutes } from '$lib/helper/enums';
	import { Button } from '../ui/button';
	import { toast } from 'svelte-sonner';
	import { Field } from '../ui/field';

	export let requestSignUp = false;
	...
	const handleGoogleSignIn = async () => {
		try {
			...
			if (data.error) {
				throw new Error(`'Failed to ${requestSignUp ? 'sign up' : 'sign in'} with Google'`);
			}
		} catch (error) {
	...
	const handleRobloxSignin = async () => {
		try {
			...
			if (data.error) {
				throw new Error(`Failed to ${requestSignUp ? 'sign up' : 'sign in'} with Roblox`);
			}
		} catch (error) {
			toast.error((error as Error).message);
		}
...
<Field class="grid gap-4 sm:grid-cols-2">
	<Button variant="outline" type="button" onclick={handleRobloxSignin} class="text-xs">
		<svg
			version="1.1"
			id="svg10"
			x="0px"
	...
	<Button variant="outline" type="button" onclick={handleGoogleSignIn} class="text-xs">
